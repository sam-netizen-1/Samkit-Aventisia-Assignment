      ...
      <div className="flex flex-col md:flex-row mb-2">
        <div className="md:order-1 md:ml-2 mb-3 md:mb-2 flex items-center gap-2 flex-shrink-0">
          ...
          <div className="md:hidden text-right flex-auto">
            <a href="#comments" className="py-1 text-blue-600 dark:text-blue-400 hover:underline">Jump to bottom</a>
          </div>
        </div>

        <h1 className="text-2xl font-semibold break-words flex-auto mr-0 mb-2 dark:text-white pull-request-title">
          <span>{title}</span>
          <span className="text-gray-500 dark:text-gray-400 font-normal ml-2">#{number}</span>
        </h1>
      </div>

      <div className="flex flex-wrap items-center mt-2">
        <div className="flex-shrink-0 mb-2 mr-2">
          <span className={`inline-flex items-center px-2 py-1 rounded-full text-sm font-medium ${getStateColor()}`}>
            {state === 'MERGED' && (
              <svg height="16" className="mr-1" viewBox="0 0 16 16" version="1.1" width="16" aria-hidden="true">
        ...
        <div className="flex-auto min-w-0 mb-2">
          <a href={author.url} className="font-medium text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400">
            {author.login}
          </a>
          ...
          {state === 'MERGED' && (
            <>
              <span className="text-gray-600 dark:text-gray-400">1 commit into </span>
              <span className="inline-flex items-center">
                <a href={`/${baseRepository.owner.login}/${baseRepository.name}/tree/${baseRefName}`} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 no-underline">
                  <span className="truncate">{baseRepository.owner.login}</span>:<span className="truncate">{baseRefName}</span>
                </a>
              </span>
              <span className="text-gray-600 dark:text-gray-400"> from </span>
              <span className="inline-flex items-center">
                <a href={`/${headRepository.owner.login}/${headRepository.name}/tree/${headRefName}`} className="text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400 no-underline">
                  <span className="truncate">{headRepository.owner.login}</span>:<span className="truncate">{headRefName}</span>
                </a>
              </span>
            </>
          )}

          <RelativeTime datetime={createdAt} className="text-gray-600 dark:text-gray-400 whitespace-nowrap" title={formatDate(createdAt)}>
